...
from vinyl_detector import DEFAULT_DRY_RUN as DEFAULT_VINYL_DRY_RUN
...
def main(vinyl_dry_run, vinyl_interval, song_interval):
    vinyl_thread = threading.Thread(target=run_vinyl_detector, args=(vinyl_dry_run, vinyl_interval, Fore.RED))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--vinyl-dry-run', action='store_true', help='Perform a dry run without querying gemini', default=DEFAULT_VINYL_DRY_RUN, required=False)
    ...
    main(args.vinyl_dry_run, args.vinyl_interval, args.song_interval)
